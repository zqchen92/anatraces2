...
void lisa_settings::loadSettings(TString name){
    ...
    fIsMWD = env.GetValue("Enable.energyCalculation.algorithm",false);
    ...
    fFebexTraceL = env.GetValue("Febex4.trace.length",1000);
...
void lisa_settings::printSettings(){
    ...
    std::cout<<"Febex4.trace.length = "<<getFebexTraceL()<<std::endl;
